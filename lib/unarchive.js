		...
		async.waterfall([
			...
			function backupOldAddon(callback) {
				...
				if (self.options.backupOld) {
					var oldAddonDir = path.join( extractDir, 'Interface' );
					var today = new Date();
					var distAddonDir = oldAddonDir + ".obsolete." + today.getYear() + (today.getMonth() + 1 ) + today.getDate() + today.valueOf();
					abUtil.renameDir(oldAddonDir, distAddonDir, function (e) {

						if (e === 'EPERM') {
							callback('备份需要使用管理员身份启动，为保护您的文件，流程终止！');
						}else{
							callback(null);
						}
					});
